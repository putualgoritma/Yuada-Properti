...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('global.accountlock.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.accountlocks.update", [$accountlock->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group {{ $errors->has('account_id') ? 'has-error' : '' }}">
                ...
                <select name="account_id" class="form-control">
                    ...
                    @foreach ($accounts as $account)
                        <option value="{{ $account->id }}"{{ $accountlock->account_id == $account->id ? ' selected' : '' }}>
