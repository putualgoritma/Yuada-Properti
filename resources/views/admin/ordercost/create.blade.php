...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.ordercost.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
                        ...
                        <tbody>
                            @foreach (old('products', ['']) as $index => $oldProduct)
                                <tr id="product{{ $index }}">
                                    ...
                                    <td>
                                        <input type="number" name="cost[]" class="form-control cost_list" value="{{ old('cost.' . $index) ?? '0' }}" />
                                    </td>
                                    {{-- <td>
...
<script>
  $(document).ready(function(){
    ...
    $(document).on("keyup", "input.cost_list" , function() {
        // alert('sss')
        let data_key = $(this).closest('tr').attr('id');
        // let qty = $('tr#'+data_key+' input.qty_list').val();
