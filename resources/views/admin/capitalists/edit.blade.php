...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('global.capitalist.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.capitalists.update", [$capitalist->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group {{ $errors->has('register') ? 'has-error' : '' }}">
                <label for="register">{{ trans('global.capitalist.fields.register') }}*</label>
                <input type="date" id="register" name="register" class="form-control" value="{{ old('register', isset($capitalist) ? $capitalist->register : '') }}" required>
                ...
                <p class="helper-block">
                    {{ trans('global.capitalist.fields.register_helper') }}
                </p>
            </div>
            <div class="form-group {{ $errors->has('code') ? 'has-error' : '' }}">
                <label for="code">{{ trans('global.capitalist.fields.code') }}*</label>
                <input type="text" id="code" name="code" class="form-control" value="{{ old('code', isset($capitalist) ? $capitalist->code : '') }}" required>
                @if($errors->has('code'))
                    <em class="invalid-feedback">
                        {{ $errors->first('code') }}
                    </em>
            ...
            <div class="form-group {{ $errors->has('type') ? 'has-error' : '' }}">
                ...
                <select name="type" class="form-control">
                    <option value="capitalist"{{(old('type', $capitalist->type) == 'capitalist' ? 'selected' : '')}}>Pemodal</option>                    
                </select>
                @if($errors->has('type'))
                    <em class="invalid-feedback">
                        {{ $errors->first('type') }}
