...
<div class="card">
    <div class="card-header">
        {{ trans('global.project.title_singular') }} {{ trans('global.list') }}
    </div>
                ...
                <tbody>
                    @foreach($projects as $key => $project)
                        <tr data-entry-id="{{ $project->id }}">
                            ...
                            <td>
                                {{ $project->register?? '' }}
                                {{-- @foreach($project->roles as $key => $item)
                                    <span class="badge badge-info">{{ $item->title }}</span>
                                @endforeach --}}
                            </td>
                            <td>
                                @can('user_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.contructioncost.create', ['project_id'=>$project->id]) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                @can('user_edit')
