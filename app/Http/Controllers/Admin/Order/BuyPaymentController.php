...
class BuyPaymentController extends Controller
{
    use TraitModel;
    public function index(Request $request)
    {
        ...
        //return $orders;
        return view('admin.buypayment.index', compact('orders', 'customers'));
    }
    public function create(Request $request)
    {
        ...
        return view('admin.buyPayment.create', compact('orders', 'accounts', 'code'));
    }
    public function store(Request $request)
    {
        ...
        // dd($ledger);
        // dd($request->all());
        $data = array_merge($request->all(), ['account_id' => '19', 'account_pay' => $request->account_id, 'customer_id' => $order->customers_id, 'label' => $memo, 'order_id' => $order->id, 'amount' => $total, 'type' => 'payable', 'status' => 'approved']);
        // dd($data);
        ...
        $payreceivable->ledgers()->attach($ledger_id, ['type' => 'D', 'status' => 'approved', 'code' => $code, 'label' => $memo, 'memo' => $request->input('memo'), 'register' => $request->input('register'), 'amount' => $total, 'account_id' => '19']);
        $payreceivable->ledgers()->attach($ledger_id, ['type' => 'C', 'status' => 'approved', 'code' => $code, 'label' => $memo, 'memo' => $request->input('memo'), 'register' => $request->input('register'), 'amount' => $total, 'account_id' => $request->account_id]);

        // dd($payreceivable);
        return redirect()->route('admin.buypayment.index');
    }
}
