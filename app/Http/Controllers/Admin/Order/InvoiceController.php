...
class InvoiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //return $orders;
        return view('admin.invoice.index', compact('orders', 'customers'));
    }
    public function create(Request $request)
    {
        ...
        $accounts = Account::select('*')
            ...
            ->get();
        return view('admin.invoice.create', compact('products', 'customers', 'accounts', 'code'));
    }
    public function store(StoreOrderRequest $request)
    {
        ...
        if ($request->paymentType == "cash") {
            ...
            $warehouses_id = 1;
            //set order
            $data = array_merge($request->all(), ['total' => $total, 'type' => 'pi', 'status' => 'approved', 'ledgers_id' => $ledger_id, 'customers_id' => $customers_id, 'payment_type' => 'cash']);
            ...
            $data = array_merge($request->all(), ['total' => $total, 'type' => 'pi', 'status' => 'approved', 'ledgers_id' => $ledger_id, 'customers_id' => $request->customers_id, 'payment_type' => 'credit']);
            $order = Order::create($data);
    ...
    public function createByOrder(Request $request)
    {
        ...
        $accounts = Account::select('*')
            ->where('accounts_group_id', 1)
            ->get();
        return view('admin.invoice.createByOrder', compact('orders', 'accounts', 'code'));
    }
    public function storeByOrder(Request $request)
    {
        ...
        if ($request->paymentType == "cash") {
            ...
            $warehouses_id = 1;
            //set order
            $data = array_merge($request->all(), ['total' => $order->total, 'type' => 'pi', 'status' => 'approved', 'ledgers_id' => $ledger_id, 'customers_id' => $customers_id, 'payment_type' => 'cash']);
            $order = Order::create($data);
            //set order products
            // dd("ssss");
            foreach ($products as $product) {
            ...
            $data = array_merge($request->all(), ['total' => $order->total, 'type' => 'pi', 'status' => 'approved', 'ledgers_id' => $ledger_id, 'customers_id' => $order->customers_id, 'payment_type' => 'credit']);
            $order = Order::create($data);
