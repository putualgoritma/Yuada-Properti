...
class SaleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //return $orders;
        return view('admin.sale.index', compact('orders', 'customers'));
    }
    public function create(Request $request)
    {

        $last_code = $this->get_last_code('sale');
        $code = acc_code_generate($last_code, 8, 3);
        $products = Product::with('units')->where('type', '=', "property")->where('status', '=', "progress")->get();
        $customers = Customer::select('*')
            ->where('def', '=', '0')
            ->where('type', '=', 'buyer')
            ->get();
        $accounts = Account::select('*')
            ...
            ->get();
        return view('admin.sale.create', compact('products', 'customers', 'accounts', 'code'));
    }
    public function store(StoreOrderRequest $request)
    {
        ...
        $data = array_merge($request->all(), ['total' => $total, 'type' => 'so', 'status' => 'pending', 'ledgers_id' => 0, 'customers_id' => $request->customers_id, 'payment_type' => 'credit']);
    ...
    public function cancel(Request $request)
    {
        ...
        $accounts = Account::select('*')
            ->where('accounts_group_id', 1)
            ->get();
        return view('admin.sale.cancel', compact('accounts', 'payreceivable', 'order'));
    }
